...
# Startup Event (Web Service Only)
@asynccontextmanager
async def lifespan(app: FastAPI):
    # Reset the cache directory, delete cache files
    reset_cache_dir()

    # Initialize the clients (WITHOUT bot mode)
    await initialize_clients_web_only()
...
# Modified clients initialization for web service only
async def initialize_clients_web_only():
    """Initialize clients for web service without starting bot mode"""
    from utils.clients import initialize_clients as original_initialize_clients
    from utils.directoryHandler import loadDriveData, init_drive_data
    from config import MAIN_BOT_TOKEN
    
    logger.info("Initializing Clients for Web Service Only")
    
    # Temporarily disable bot token to prevent bot initialization
    original_bot_token = MAIN_BOT_TOKEN
    import config
    config.MAIN_BOT_TOKEN = None
    
    try:
        # Initialize clients without bot
        await original_initialize_clients()
    finally:
        # Restore original bot token
        config.MAIN_BOT_TOKEN = original_bot_token
    
    logger.info("Web Service clients initialized successfully")
